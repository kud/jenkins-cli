...
const getClient = async () => {
  ...
  if (globalOpts.debugConfig) console.error('--- debug-config program.opts() ---', globalOpts);
  const fileConfig = loadConfig();
  if (globalOpts.debugConfig) console.error('--- debug-config fileConfig ---', fileConfig);
  ...
  if (globalOpts.debugConfig) {
    try {
      ...
      let raw = '';
      try { raw = fs.readFileSync(CONFIG_FILE, 'utf8'); } catch (_) { raw = '(missing)'; }
...
program.command('config')
  ...
  .action((action, name, options) => {
    try {
      ...
      if (action === 'use') {
        if (!name) { console.error('config use <name>'); process.exit(1); }
        useServer(name);
        console.log(`Current server set to '${name}'.`);
        return;
      }
      if (action === 'remove-server') {
        if (!name) { console.error('config remove-server <name>'); process.exit(1); }
        removeServer(name);
  ...
  .action(async (job, cmd) => {
    try {
      ...
      for (const kv of cmd.param) {
        ...
        if (idx === -1) { console.error('Param must be key=value: ' + kv); process.exit(1); }
        const k = kv.slice(0, idx); const v = kv.slice(idx+1);
        params[k] = v;
      }
    ...
    } catch (e) { formatError(e); process.exit(1); }
  });

program.command('stop <job> <buildNumber>')
  .description('Stop/abort a running build')
  .action( async (job, buildNumber) => {
    try { const client = await getClient(); const res = await client.stopBuild(job, buildNumber); console.log(res); } catch (e) { formatError(e); process.exit(1); }
  });

program.command('queue')
  .description('List queue items')
  .action( async () => {
    try { const client = await getClient(); const q = await client.getQueue(); const jsonFlag = program.opts().json; if (jsonFlag) console.log(JSON.stringify(q, null, 2)); else console.log((q.items||[]).map(i=>`${i.id}\t${i.task?.name||''}\tblocked=${i.blocked} buildable=${i.buildable}`).join('\n')); } catch (e) { formatError(e); process.exit(1); }
  });

program.command('queue-cancel <id>')
  .description('Cancel a queue item by id')
  .action( async (id) => { try { const client = await getClient(); const res = await client.cancelQueueItem(id); console.log(res); } catch (e) { formatError(e); process.exit(1); } });

program.command('test-report <job> <buildNumber>')
  .description('Fetch JUnit test report summary for a build')
  .action( async (job, buildNumber) => {
    try { const client = await getClient(); const rep = await client.getTestReport(job, buildNumber); const jsonFlag = program.opts().json; if (jsonFlag) console.log(JSON.stringify(rep, null, 2)); else { const total = rep.totalCount; const fail = rep.failCount; const skip = rep.skipCount; console.log(`# Tests: ${total}  Failed: ${fail}  Skipped: ${skip}`); if (rep.suites) { rep.suites.slice(0,5).forEach(s=> console.log(`- ${s.name} ${s.cases?.length||0} cases`)); if (rep.suites.length>5) console.log(`... (${rep.suites.length-5} more suites)`); } } } catch (e) { formatError(e); process.exit(1); }
  });

program.command('stages <job> <buildNumber>')
  .description('Fetch pipeline stages (workflow-api plugin required)')
  .action( async (job, buildNumber) => {
    try { const client = await getClient(); const data = await client.getPipelineStages(job, buildNumber); const jsonFlag = program.opts().json; if (jsonFlag) console.log(JSON.stringify(data, null, 2)); else { if (data.stages) { data.stages.forEach(s=> console.log(`${s.id}\t${s.name}\t${s.status}\t${Math.round((s.durationMillis||0)/1000)}s`)); } else console.log('No stages'); } } catch (e) { formatError(e); process.exit(1); }
  });
