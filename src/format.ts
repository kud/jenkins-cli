...
export function formatLogsChunk(chunk) {
  ...
  return text.split(/\n/).map(line => {
    ...
    if (/^\s*at\s+[\w.$]+\(/.test(line)) return chalk.dim.red('  ↳ ' + line.trim());
    if (/^\s*\.{3}\s*\d+\s+more/i.test(line)) return chalk.dim.red('  ⋯ ' + line.trim());
    ...
    line = line.replace(/\b(\d{1,3}(?:[,\s]\d{3})*(?:\.\d+)?)\s*(?:MB|GB|KB|bytes?|ms|seconds?|minutes?|hours?)\b/gi, 
