...
export async function runTUI(client, { job, limit = 10 }) {
  ...
  screen.key('r', async () => {
    if (loading) return; await refreshBuilds();
    if (selectedBuildNumber) loadLogs(selectedBuildNumber);
  });

  screen.key('f', async () => {
    follow = !follow;
    setStatus((follow ? 'Follow ON' : 'Follow OFF') + ' q:quit r:refresh f:follow');
    if (follow && selectedBuildNumber) startFollow(selectedBuildNumber);
    if (!follow && abortController) abortController.abort();
  });
