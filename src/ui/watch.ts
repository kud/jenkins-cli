...
export async function runWatch(client: JenkinsClient, { job, refreshInterval = 5000, forceBasicColor = false, noTerminfo = false }: WatchOptions) {
  ...
  const getStatusIcon = (result: string, building: boolean) => {
    if (building) return '{blue-fg}RUNNING 🔄{/}';
    switch (result) {
  ...
  const updateControlBar = () => {
    ...
    controlBar.setContent(
      `  Auto-refresh: ${autoRefreshText} (${intervalText}) | Follow: ${followText}${streamingText} | ` +
  ...
  const startFollow = async () => {
    if (!latestBuild || isStreaming) return;
  ...
  const getRefreshInterval = () => {
    if (!latestBuild) return refreshInterval;
