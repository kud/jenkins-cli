...
test("App mounts, loads builds, and renders logs (single-job mode)", async () => {
    const { lastFrame, unmount } = render(_jsx(App, { client: stubClient(), jobSearchLimit: 0, buildsLimit: 10, preselectJob: "demo", jobsFilter: ["demo"], singleJobMode: true }));
    await wait(150);
...
test("pressing 3 focuses the Logs pane", async () => {
    const { lastFrame, stdin, unmount } = render(_jsx(App, { client: stubClient(), jobSearchLimit: 0, buildsLimit: 10, preselectJob: "demo", jobsFilter: ["demo"], singleJobMode: true }));
    await wait(120);
    stdin.write("3");
    await wait(60);
