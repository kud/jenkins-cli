...
export const loadConfig = () => {
    try {
        ...
        const fixLink = (v) => typeof v === 'string' && /^\[[^\]]+\]\([^\)]+\)$/.test(v) ? v.replace(/^\[[^\]]+\]\(([^\)]+)\)$/, '$1') : v;
        if (parsed && typeof parsed === 'object') {
            if (parsed.url)
                parsed.url = fixLink(parsed.url);
            if (parsed.servers) {
                for (const k of Object.keys(parsed.servers)) {
                    const srv = parsed.servers[k];
                    if (srv && srv.url)
                        srv.url = fixLink(srv.url);
                }
            }
        }
        return parsed;
...
export const resolveConfig = (overrides = {}) => {
    ...
    let base = {};
    if (file.servers) {
        const srv = pickServer(file, overrides.server || env.server);
        if (srv)
            base = { ...srv };
    }
    else {
...
export const useServer = (name) => {
    const file = loadConfig();
    if (!file.servers || !file.servers[name])
