...
export function formatLogsChunk(chunk) {
    ...
    return text.split(/\n/).map(line => {
        ...
        if (/UNSTABLE/i.test(line))
            return chalk.bold.yellow('⚠️  ' + line);
        if (/ABORTED/i.test(line))
            return chalk.bold.gray('✂️  ' + line);
        ...
        if (/^\s*[\+>]*\s*(npm|yarn|pip|mvn|gradle|make|cargo|go build)/.test(line))
            return chalk.blue('🔨 ' + line);
        // Diff style (enhanced)
        if (/^@@ .* @@/.test(line))
