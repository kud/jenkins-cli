...
export async function runInteractive(client, { jobSearchLimit = 0, buildsLimit = 15, forceBasicColor = false, preselectJob = null, noTerminfo = false, jobsFilter = null }) {
    ...
    let analysisBox = null; // AI analysis modal box
    let aiChildProcess = null; // streaming child process for AI
    let analysisUpdateTimer = null; // throttle timer for streaming updates
    let analysisRawMarkdown = ''; // accumulated raw markdown from model
    let analysisLastRender = 0; // last render timestamp
    let pipelineBox = null; // pipeline diagram modal
    let pipelineLoading = false;
    let buildActionBusy = false; // prevent concurrent stop/restart
    let actionBox = null; // build actions modal
    let inputMode = 'none';
    ...
    const stripHtml = (html) => {
        ...
        text = text.replace(/&([a-zA-Z]+);/g, (_, entity) => {
    ...
    const refreshJobs = async () => {
        if (analysisBox) {
            setStatus('{cyan-fg}AI analysis open (jobs may refresh){/}');
        }
        try {
            if (singleJobMode) {
    ...
    const applyBuildFilters = () => {
        ...
        if (sortAsc)
            ...
            base.sort((a, b) => b.number - a.number);
        ...
        buildsWithScores = buildsWithScores.filter(item => {
            ...
            if (resultFilter !== 'ALL') {
                if (resultFilter === 'FAILED' && state !== 'FAILURE')
                    return false;
                if (resultFilter === 'SUCCESS' && state !== 'SUCCESS')
                    return false;
                if (resultFilter === 'RUNNING' && state !== 'RUNNING')
                    return false;
            }
            // Search filter
            if ((buildTextFilter || buildSearchQuery) && score === 0)
                return false;
            return true;
        });
    ...
    const refreshBuilds = async () => {
        if (analysisBox) {
            setStatus('{cyan-fg}AI analysis open{/}');
        }
        if (!currentJob)
            return;
        try {
    ...
    const updateMetadataBox = (num) => {
        ...
        if (logStats) {
            ...
            if (logStats.errors > 0)
                statParts.push(`{bold}{yellow-fg}Errors:{/} {red-fg}${logStats.errors}{/}`);
            if (logStats.warnings > 0)
                statParts.push(`{bold}{yellow-fg}Warnings:{/} {yellow-fg}${logStats.warnings}{/}`);
            statsLine = `\n${statParts.join('  ')}`;
        }
        metadataBox.setContent(`{bold}{yellow-fg}Build:{/} {white-fg}#${num}{/}  {bold}{yellow-fg}Status:{/} {${statusColor}-fg}${status}{/}  {bold}{yellow-fg}Duration:{/} {white-fg}${duration}{/}\n` +
    ...
    const loadLogs = async (num) => {
        if (analysisBox) {
            setStatus('{cyan-fg}AI analysis open (logs still load){/}');
        }
        if (!currentJob || !num)
            return;
        if (pipelineBox) {
            closePipelineModal({ silent: true });
        }
        if (actionBox) {
    ...
    const showArtifacts = async () => {
        ...
        const b = filteredBuilds[selIdx];
        if (!b) {
            setStatus('No build selected for artifacts');
            return;
        }
        artifactMode = true;
        ...
        if (artifacts.length === 0)
            artifactBox.setItems(['{gray-fg}No artifacts{/}']);
        else
            artifactBox.setItems(artifacts.map(a => a.relativePath));
        ...
        artifactBox.key(['escape', 'a'], () => { if (artifactBox) {
            artifactBox.destroy();
            artifactBox = null;
            artifactMode = false;
            screen.render();
    ...
    const startFollow = async (num) => {
        if (analysisBox) {
            setStatus('{cyan-fg}AI analysis open (following logs){/}');
        }
        if (!currentJob || !num)
            return;
        if (abortController)
    ...
    const startJobSearch = () => {
        if (singleJobMode || (helpBox || artifactBox || pipelineBox || actionBox))
            return;
        searchMode = true;
    ...
    screen.key(['L', 'S-l'], () => {
        ...
        input.on('submit', async (val) => {
            ...
            if (Number.isFinite(n) && n >= 0) {
                jobSearchLimit = n;
                setStatus('Job limit set to ' + (n === 0 ? 'UNLIMITED' : n));
                await refreshJobs();
            }
            else {
    ...
    // Build search modes
    screen.key('b', () => {
        if (helpBox || artifactBox || pipelineBox || actionBox)
            return;
        if (inputMode === 'classic')
            deactivateClassicTyping();
        if (isTyping())
    ...
    const closePipelineModal = (options = {}) => {
        if (pipelineBox) {
            pipelineBox.destroy();
            pipelineBox = null;
        }
    ...
    const closeActionModal = (options = {}) => {
        if (actionBox) {
            actionBox.destroy();
            actionBox = null;
        }
    ...
    const stageStatusBadge = (rawStatus) => {
    ...
    const collectStageChildren = (stage) => {
        const children = [];
        ...
        if (Array.isArray(stage?.stageFlowNodes) && stage.stageFlowNodes.length) {
            const noteworthyNodes = stage.stageFlowNodes.filter((node) => {
    ...
    const renderStageLines = (stage, prefix, isLast) => {
        const lines = [];
    ...
    const buildPipelineDiagram = (pipeline, jobName, buildNumber) => {
        if (!pipeline)
            return '{gray-fg}No pipeline data available{/}';
        const lines = [];
    ...
    const togglePipelineView = async () => {
        ...
        pipelineBox.key(['escape', 'p', 'x'], () => closePipelineModal());
        pipelineBox.focus();
        screen.render();
    ...
    const stopCurrentBuild = async () => {
        if (buildActionBusy) {
            setStatus('{yellow-fg}Another build action in progress{/}');
            return;
        }
        if (!currentJob || !logCurrentBuild) {
            setStatus('{yellow-fg}Select a running build to stop{/}');
            return;
        }
        const currentBuild = builds.find(b => b.number === logCurrentBuild);
        if (!currentBuild) {
            setStatus('{red-fg}Build not found in list{/}');
            return;
        }
    ...
    const rerunCurrentJob = async () => {
        ...
        if (!currentJob) {
            setStatus('{yellow-fg}Select a job to rerun{/}');
            return;
        }
        buildActionBusy = true;
        try {
    ...
    const openActionsMenu = () => {
        ...
        actionBox._actions = actions;
        actionBox.key(['escape', 'q'], () => { closeActionModal(); });
    ...
    const performAIAnalysis = async (model) => {
        // Prevent multiple modals
        if (analysisBox) {
            setStatus('{yellow-fg}AI analysis already running{/}');
            return;
        }
        try {
            ...
            const whichResult = new Promise(res => {
            ...
            if (!model) {
                ...
                modelsProc.on('close', () => {
                    ...
                    modelSelectBox.key(['enter'], () => {
                        const idx = modelSelectBox.selected;
                        const chosen = lines[idx];
                        modelSelectBox.destroy();
                        screen.render();
                        performAIAnalysis(chosen);
                    });
                    modelSelectBox.key(['escape', 'q'], () => {
                        modelSelectBox.destroy();
                        screen.render();
            ...
            if (buildObj)
                contextLines.push(`Build: #${buildObj.number} Result: ${buildObj.building ? 'RUNNING' : (buildObj.result || 'UNKNOWN')}`);
            if (buildObj && buildObj.timestamp)
                contextLines.push(`Started: ${new Date(buildObj.timestamp).toISOString()}`);
            if (buildObj && buildObj.duration)
                contextLines.push(`Duration(ms): ${buildObj.duration}`);
            contextLines.push('---');
            ...
            analysisBox.key(['up', 'k'], () => scrollModal(-1));
            analysisBox.key(['down', 'j'], () => scrollModal(1));
            analysisBox.key(['pageup'], () => scrollModal(-modalPageSize()));
            analysisBox.key(['pagedown'], () => scrollModal(modalPageSize()));
            ...
            const renderNow = () => {
                ...
                display = display.replace(/^\|\s*([^|\n]+)\|\s*([^|\n]+)\|\s*([^|\n]+?)\s*\|$/gm, (_line, stage, state, detail) => {
                    const stageLabel = stage.trim();
                    const stateLabel = state.trim();
                    const detailLabel = detail.trim();
                    let stateColor = 'white-fg';
                    if (/fail|error|abort|mismatch|blocked/i.test(stateLabel))
                        stateColor = 'red-fg';
                    else if (/skip|defer|pending/i.test(stateLabel))
                        stateColor = 'yellow-fg';
                    else if (/pass|success|healthy|recovered|ok/i.test(stateLabel))
                        stateColor = 'green-fg';
                ...
                display = display.replace(/^-\s*(💥 Root Cause|📜 Evidence|📡 Impact|🛠 Fix|✅ Healthy Signals|🔭 Watch Next):(.*)$/gm, (_line, label, rest) => {
            ...
            const seenTextChunkIds = new Set();
            const markTextEventProcessed = (event, extraIds = []) => {
                if (!event || typeof event !== 'object') {
                    return false;
                }
                const extra = extraIds.filter((id) => typeof id === 'string' && id.length > 0);
                const identifiers = [
                    event.part?.id,
            ...
            const gatherText = (value, depth = 0, seen = new WeakSet()) => {
                ...
                if (typeof value === 'object') {
                    ...
                    for (const key of priorityKeys) {
                        if (key in value) {
                            out += gatherText(value[key], depth + 1, seen);
                        }
                    }
                    if (!out) {
                ...
                try {
                    ...
                    switch (type) {
                        ...
                        case 'response': {
                            ...
                            text = gatherText(event.response?.message?.content ??
            ...
            analysisBox.key(['escape', 'q', 'x'], () => { closeAnalysisModal(); });
            analysisBox.key(['D'], () => {
                try {
    ...
    screen.key(['C-a'], () => {
        if (isTyping() || helpBox || artifactBox || pipelineBox || actionBox || analysisBox)
            return;
        openActionsMenu();
    });
    ...
    screen.key(['left'], () => {
        if (isTyping())
            return;
        if (artifactBox || helpBox || pipelineBox || actionBox)
    ...
    screen.key('1', () => { if (isTyping())
        return; if (!artifactBox && !helpBox && !pipelineBox && !actionBox && !singleJobMode) {
        jobsBox.focus();
        showPaneFeedback('jobs');
        setStatus();
        screen.render();
    ...
    screen.key('2', () => { if (isTyping())
        return; if (!artifactBox && !helpBox && !pipelineBox && !actionBox) {
        buildsBox.focus();
        showPaneFeedback('builds');
        setStatus();
        screen.render();
